...
class CustomDataset_test(Dataset):
    def __init__(self, dataframe, sr=32000, duration=5, segment_duration=1, transform=None):
        self.dataframe = dataframe
        self.sr = sr
        self.duration = duration
        self.segment_duration = segment_duration
        self.segment_length = sr * segment_duration
        self.transform = transform
    ...
    def __getitem__(self, index):
        ...
        segment_samples = self.segment_duration * self.sr
        segments = []
...
def validation(model, criterion, val_loader, device):
    ...
    with torch.no_grad():
        for features, labels in tqdm(val_loader):
            ...
            val_loss.append(criterion(logits, labels).item())

            all_labels.append(labels.cpu().numpy())
            all_probs.append(torch.sigmoid(logits).cpu().numpy())  # 시그모이드 함수를 적용하여 확률로 변환
...
def inference(model, test_loader, device):
    ...
    with torch.no_grad():
            ...
            #raise ValueError("Number of test samples must be at least 10.")
 
        for segments_list in tqdm(test_loader):
            batch_predictions = []
            
            for segment in segments_list:
                # segment는 (channels, segment_length) 형태의 텐서입니다.
                segment = segment.float().to(device)  # (1, channels, segment_length) 형태로 변경
                #print(segment.shape) # 1 1 32000
                probs = model(segment)
                batch_predictions.append(probs)

            # 5개 세그먼트의 예측 결과 평균 계산
            stack = torch.stack(batch_predictions, dim=1) # 1 5 2
            #print(stack.shape)
            avg_probs = torch.mean(stack, dim=1) # 1 16 1 32000
            #print(avg_probs.shape) # 1, 2
            avg_probs = avg_probs.cpu().detach().numpy()
            #predictions.append(avg_probs)
            predictions += avg_probs.tolist()
...
submit.to_csv('./baseline_submit_zero_5%_testdecision_sina_e8.csv', index=False)