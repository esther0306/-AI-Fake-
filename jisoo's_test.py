...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.label is not None:
            return waveform, self.label[index]
        return waveform

class CustomDataset_test(Dataset):
    def __init__(self, dataframe, sr=32000, duration=5):
        self.dataframe = dataframe
        self.sr = sr
        self.duration = duration
        self.segment_length = sr * duration
        self.resampler = torchaudio.transforms.Resample(orig_freq=sr, new_freq=32000)

    def __len__(self):
        return len(self.dataframe)

    def __getitem__(self, index):
        waveform, orig_sr = torchaudio.load(self.dataframe[index])#audio_path)

        segment_samples = 1 * 32000
        segments = []
        for start in range(0, waveform.size(1), segment_samples):
            end = start + segment_samples
            segment = waveform[:, start:end]
            if segment.size(1) == segment_samples:  # Ensure segment is the correct length
                segments.append(segment)

        segments_tensor = torch.stack(segments)
        mean_segment = torch.mean(segments_tensor, dim=0)
...
def mixup_data(x, y, alpha=1.0):
    ...
    for i in range(batch_size):
        if np.array_equal(y_a[i], [0, 1]) and np.array_equal(y_b[i], [0, 1]):
            mixed_y[i] = torch.tensor([0, 1])  # real, real
        elif np.array_equal(y_a[i], [1, 0]) and np.array_equal(y_b[i], [1, 0]):
...
class MixupDataLoader:
    def __init__(self, data_loader, alpha=1.0, fixed_lam=None, save_mixup_examples=False, num_examples_to_save=5):
        self.data_loader = data_loader
        self.alpha = alpha
        self.fixed_lam = fixed_lam
        self.save_mixup_examples = save_mixup_examples
    ...
    def __iter__(self):
        for x, y in self.data_loader:
            ...
            # Save mixup examples
            if self.save_mixup_examples and self.saved_examples < self.num_examples_to_save:
                batch_size = x.size(0)
                num_to_save = min(self.num_examples_to_save - self.saved_examples, batch_size)
                save_indices = random.sample(range(batch_size), num_to_save)
                for save_idx in save_indices:
                    output_filename = f"mixup_example_{self.saved_examples + 1}.wav"
                    torchaudio.save(output_filename, mixed_x[save_idx].cpu(), CONFIG.SR)
                    self.saved_examples += 1
                    if self.saved_examples >= self.num_examples_to_save:
                        break
...
def train_supervised(model, optimizer, scheduler, train_loader, val_loader, device):
    ...
    for epoch in range(1, CONFIG.N_EPOCHS+ 1):
        ...
        for features, y_a, lam in tqdm(iter(train_loader)):
...
test_dataset = CustomDataset_test(test_feat, 32000, 5)
...
submit.to_csv('./baseline_submit_onlycrop.csv', index=False)
'''
...
submit.to_csv('./baseline_submit_base_ctest.csv', index=False)
